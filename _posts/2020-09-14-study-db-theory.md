...
## 파일 시스템의 문제점
 - 무결성을 보장하지 못하여 데이터간 불일치성 발생
 - 파일 분석이 필요할 때 마다 별도의 프로그램 작성 필요
 - 공유된 파일의 경우, 많은 사용자가 동시에 저장하면 마지막에 저장한 사람의 의도만 반영
 - 데이터 파일의 복사나 참조가 손쉬워 핵심 정보가 유출 가능성 큼
 - 중복된 파일로 존재하기 때문에 파일을 유츌시킨 사람을 확인하기 힘듦
...
## 데이터베이스 시스템의 인터페이스(interface)
 - 응용 인터페이스 : 특정 외부 단계와 개념 단계 사이의 사상(mapping)을 위한 인터페이스, 뷰
